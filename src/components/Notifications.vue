...
<template>
    <div class="bg-grey-300 w-fit">
        <div v-for="(sampleNotification,index) in sampleNotifications" :key="index">
            <OneNotification v-if="Object.keys(sampleNotification).length !== 0" :anotification="sampleNotification">
            </OneNotification>
            <div v-else class="min-h-[64px] px-5 py-3" :class="messageRead === false && 'bg-read'">
